...
echo "Test A72 for huge-count.pl"
echo "Running huge-count.pl --token token.regex --nontoken nontoken.regex --stop stoplist test-A72.output test-A72.data"

huge-count.pl --token token.regex --nontoken nontoken.regex --stop stoplist test-A72.output test-A72.data

# testing count

foreach i (1 2 3 4)
	sort test-A72.output/test-A72.data$i.bigrams > t0
...
sort test-A72.output/huge-count.output > t0
...
/bin/rm -f -r var1 t0 t1 test-A72.output
